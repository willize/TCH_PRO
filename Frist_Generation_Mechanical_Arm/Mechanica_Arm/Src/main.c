...
extern 	u16	Y_num6;
extern 	u16	X_num7;
extern  u16 Wait_time_s;
float  Z_Accel_Freq	 [1000]={0};
u16 	 Z_Accel_Period[1000]={0};
float  XY_Accel_Freq	[50]={0};
u16 	 XY_Accel_Period[50]={0};

static float Glue_Machine_High = 10.5f;
static float Down_Distance = 5.0f;
static float Y_Tool_Mov_Distance = 71.93f;
static float X_Tool_Groove_Distance = 26.7f;
static float Y_Tool_Groove_Distance = 24.8f;

static float Y_Glue_Tool = 1.65f;    //打胶工装与放置工装的差距
static float X_Glue_Tool = 1.41f;   
  ...
  while (1)
  {
		...
		if (main_sta&0x0008)
			{	
				...
				Y_Control(Y_LR_Tool_Mov_Distance+0.30);
				delay_ms(1500);
		...
		if ( (X_TI_stuta&0x0010) && (Y_TI_stuta&0x0010) && (Z_TI_stuta&0x0010))//放置左边的物品
		{
			...
			if (main_sta&0x0010)
				{	
					...
					Z_Control(Glue_Machine_High);
					delay_ms(2500);
					X_Control( X_Glue_Tool);
					Y_Control( Y_Tool_Mov_Distance + Y_Glue_Tool);
					delay_ms(5000);
					Z_downn_point_2=1;
				...
				if ( (X_TI_stuta&0x0040) && (Y_TI_stuta&0x0040) && (Z_TI_stuta&0x0040))//夹取打胶右面的物品
				{
					...
					if (main_sta&0x0040)
						{	
							...
							X_Control( (40.0f-X_Tool_Groove_Distance-X_Glue_Tool));
							Y_Control( (50.0f-Y_Tool_Groove_Distance-Y_Glue_Tool - 0.35 ));						
						  delay_ms(2000);
							Z_Control( Z_Improve );
				...
				if ( (X_TI_stuta&0x0100) && (Y_TI_stuta&0x0100) && (Z_TI_stuta&0x0100))//放置打胶后右面的物品
				{
					...
					if (main_sta&0x0100)
						{	
							...
							X_TI_stuta=0x0200;
							Y_Control( Y_Tool_Mov_Distance - 0.25) ;
							delay_ms(2000);
							Z_Control( (Glue_Machine_High + Z_Improve - Down_Distance-0.5) );
				...
				if ( (X_TI_stuta&0x0200) && (Y_TI_stuta&0x0200) && (Z_TI_stuta&0x0200))//抓取打胶后左面的物品
				{
					...
					if (main_sta&0x0200)
						{	
							...
							Z_Control( Glue_Machine_High );
							delay_ms(3000);
							X_Control( X_Tool_Groove_Distance + X_Glue_Tool);
							Y_Control( (Y_Tool_Mov_Distance + Y_Tool_Groove_Distance + Y_Glue_Tool + 0.1));
							delay_ms(5000);
							Z_downn_point_3=1;
							main_sta=0x0400;
				...
				if ( (X_TI_stuta&0x0800) && (Y_TI_stuta&0x0800) && (Z_TI_stuta&0x0800))//放置打胶后左面的物品
				{
					...
					if (main_sta&0x0800)
						{	
							...
							X_Control( X_Glue_Tool);
							Y_Control( Y_Tool_Mov_Distance+ Y_Glue_Tool + 0.15 );
							delay_ms(1500);
							Z_Control((Glue_Machine_High + Z_Improve - Down_Distance-0.5));
